  ...
  for (tt in 1:ngen) {
    ...
    if (k[1]>0) {
      ...
      if (events$time[idx]==0) {
        ...
        idx2 <-  which((events$node==node2)*(events$time==0)==1)
        tmp <- events[-c(idx,idx2),]
        lik.new <- likcal_new(node=anc,root.included=F,events=tmp,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=nodeu,sd=sd,nodea=nodea,nodeb=nodeb,nodex=nodex,nodedistr=nodedistr,nodedistro=nodedistro,nodeniche=nodeniche,nodeback=nodeback,d=d,bias=bias,area=area)
        accept <- min(1,exp(sum(lik.old$res[lik.new$idx])-sum(lik.new$res))*k[1]/A[1]*ifelse(k[1]==1,2,1)/prior.n(n)*(N-1)*exp.r^2)
      } else {
        tmp <- events[-idx,]
        lik.new <- likcal_new(node=node,root.included=T,events=tmp,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=nodeu,sd=sd,nodea=nodea,nodeb=nodeb,nodex=nodex,nodedistr=nodedistr,nodedistro=nodedistro,nodeniche=nodeniche,nodeback=nodeback,d=d,bias=bias,area=area)
    ...
    if (runif(1)<0.05) {
      ...
      lik.new <- likcal_new(node=0,root.included=F,events=events,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=rootu,sd=sd,nodea=tmpa,nodeb=rootb,nodex=rootx,nodedistr=rootdistrtmp,nodedistro=rootdistrotmp,nodeniche=rootnichetmp,nodeback=rootback,d=d,bias=bias,area=area)
      ...
      rootdistrtmp <- rootdistrotmp[(N+1):(2*N)]
      rootdistrtmp[1] <- rootdistrtmp[1]+sum(rootdistrotmp[1:N])
      lik.new <- likcal_new(node=0,root.included=F,events=events,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=rootu,sd=sd,nodea=roota,nodeb=tmpb,nodex=rootx,nodedistr=rootdistrtmp,nodedistro=rootdistrotmp,nodeniche=rootnichetmp,nodeback=rootback,d=d,bias=bias,area=area)
      ...
      rootdistrtmp <- rootdistrotmp[(N+1):(2*N)]
      rootdistrtmp[1] <- rootdistrtmp[1]+sum(rootdistrotmp[1:N])
      lik.new <- likcal_new(node=0,root.included=F,events=events,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=rootu,sd=tmpsd,nodea=roota,nodeb=rootb,nodex=rootx,nodedistr=rootdistrtmp,nodedistro=rootdistrotmp,nodeniche=rootniche,nodeback=rootbacktmp,d=d,bias=bias,area=area)
      ...
      rootdistrtmp <- rootdistrotmp[(N+1):(2*N)]
      rootdistrtmp[1] <- rootdistrtmp[1]+sum(rootdistrotmp[1:N])
      lik.new <- likcal_new(node=0,root.included=F,events=events,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=tmpu,sd=sd,nodea=roota,nodeb=rootb,nodex=rootx,nodedistr=rootdistrtmp,nodedistro=rootdistrotmp,nodeniche=rootniche,nodeback=rootbacktmp,d=d,bias=bias,area=area)
      ...
      lik.new <- likcal_new(node=0,root.included=F,events=events,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=rootu,sd=sd,nodea=roota,nodeb=rootb,nodex=rootx,nodedistr=rootdistr,nodedistro=rootdistro,nodeniche=rootniche,nodeback=rootback,d=tmp,bias=bias,area=area)
      ...
      if (dim(events)[1]>0) {
        ...
        eventstmp$a <- tmp
        eventstmp$b <- tmp
        lik.new <- likcal_new(node=0,root.included=F,events=eventstmp,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=rootu,sd=sd,nodea=roota,nodeb=rootb,nodex=rootx,nodedistr=rootdistr,nodedistro=rootdistro,nodeniche=rootniche,nodeback=rootback,d=d,bias=bias,area=area)
