...
A1 <- 20
A2 <- 20
sim2 <- simcode(sp,mu,nspecies,A1,A2,rootu,sd,roota,rootb,rootx,d,N,r,pois.x,area,nBK)   # sim changed to sim2
while(inherits(sim2,"try-error")) {
	sim2 <- simcode(sp,mu,nspecies,A1,A2,rootu,sd,roota,rootb,rootx,d,N,r,pois.x,area,nBK)
}
...
BK <- sim2$BK
edge <- sim2$tree$edge
edge.length <- sim2$tree$edge.length
edge.prob1 <- sim2$edge.prob1
edge.prob2 <- sim2$edge.prob2
...
lik.old <- likcal_new(node=0,root.included=F,events=events,edge=edge,edge.length=edge.length,nspecies=nspecies,PO=PO,PA=PA,N=N,nodeu=rootu,sd=sd,nodea=roota,nodeb=rootb,nodex=rootx,nodedistr=rootdistr,nodedistro=rootdistro,nodeniche=rootniche,nodeback=rootback,d=d,bias=bias,area=area)
...
saveRDS(lik2.old, file = "lik2.old.RData")

saveRDS(sim2, file = "sim2.RData")
