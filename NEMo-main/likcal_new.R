likcal_new <- function (node,root.included,events,edge,edge.length,nspecies,PO,PA,N=10,nodeu,sd,nodea,nodeb,nodex,nodedistr,nodedistro,nodeniche,nodeback,d,bias,area) {  print("test 1")
  
 # Rcpp::sourceCpp("updX5.cpp")
  ...
  if (root.included) { print("test 2")
    ...
    #res <- updX5(events=events[events$node==node,],nrow(events),edget=edge.length[node],dendistrt=nodedistr,dendistrto=nodedistro,nichedistrt=nodeniche,backdistrt=nodeback,a=nodea,b=nodeb,u=nodeu,x=nodex,sd,N,d)
  ...
  if (length(des)>0) { print("test 3")
    ...
    #res <- updX5(events[events$node==j,],nrow(events),edget=edge.length[j],dendistrt=nodedistr,dendistrto=nodedistro,nichedistrt=nodeniche,backdistrt=nodeback,a=nodea,b=nodeb,u=nodeu,x=nodex,sd,N,d)
    ...
    xvec[j] <- res$x
    j<-j+1
    while(j<=dim(edge)[1] && edge[j,1]>edge[des[1],1]) { print("test 4")
      ...
      #res <- updX5(events[events$node==j,],nrow(events),edget=edge.length[j],dendistrt=dendistr[tmp,],dendistrto=dendistro[tmp,],nichedistrt=nichedistr[tmp,],backdistrt=backdistr[tmp,],a=avec[tmp],b=bvec[tmp],u=uvec[tmp],x=xvec[tmp],sd,N,d)
      dendistr[j,] <- res$dendistrt
      dendistro[j,] <- res$dendistrto
      nichedistr[j,] <- res$nichedistrt
      backdistr[j,] <- res$backdistrt
    ...
    xvec[j] <- res$x
    j<-j+1
    while(j<=dim(edge)[1] && edge[j,1]>edge[des[2],1]) {  print("test 5")
      tmp <- as.character(edge[j,1])
      res <- upd(events[events$node==j,],edget=edge.length[j],dendistrt=dendistr[tmp,],dendistrto=dendistro[tmp,],nichedistrt=nichedistr[tmp,],backdistrt=backdistr[tmp,],a=avec[tmp],b=bvec[tmp],u=uvec[tmp],x=xvec[tmp],sd,N,d)
      #res <- updX5(events[events$node==j,],nrow(events),edget=edge.length[j],dendistrt=dendistr[tmp,],dendistrto=dendistro[tmp,],nichedistrt=nichedistr[tmp,],backdistrt=backdistr[tmp,],a=avec[tmp],b=bvec[tmp],u=uvec[tmp],x=xvec[tmp],sd,N,d)
      dendistr[j,] <- res$dendistrt
